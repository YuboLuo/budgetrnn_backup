...
def convert_network(model_path: str, model_parameters: Dict[str, np.ndarray], precision: int, sensor_type: str, is_msp: bool):
    ...
    if model_type == SequenceModelType.BUDGET_RNN:
        ...
        if os.path.exists(controller_path):
            ...
            power_system = make_power_system(power_type=PowerType[sensor_type.upper()],
    ...
    num_levels = num_outputs if model_type == SequenceModelType.BUDGET_RNN else seq_length
    power_system = make_power_system(power_type=PowerType[sensor_type.upper()],
...
# model_path = './../trained_models/whale/budget/model-SAMPLE_RNN-whale-2020-09-17-18-56-21_model_best.pkl.gz'

# convert_network(model_path=model_path,model_parameters=read_by_file_suffix(model_path),sensor_type='temp',precision=10,is_msp=True)

if __name__ == '__main__':
    parser = ArgumentParser('Compresses the neural network and converts the parameters into a C header file.')
    parser.add_argument('--model-path', type=str, required=True)
    parser.add_argument('--sensor-type', type=str, required=True, choices=['bluetooth', 'temp'])

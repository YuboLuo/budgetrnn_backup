...
def execute_client(sample_freq: float, seq_length: int, max_sequences: int, data_files: DataFiles, output_file: str, budget: float, is_skip: bool, start_idx: int):
    ...
    for seq_idx, (seq_inputs, label) in enumerate(zip(input_data, labels)):
        ...
        for idx, elem in enumerate(seq_inputs):
            elem = elem[:-1]
            features = elem.split(' ')
            # print(features)
            array.append(int(features[0]))
            array.append(int(features[1]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample-freq', type=float, required=True, help='The sample frequency in Hz')
